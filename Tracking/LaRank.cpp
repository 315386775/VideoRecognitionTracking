...
LaRank::LaRank(const Config& conf, const Features& features, const Kernel& kernel) :
	...
	m_C(conf.svmC)
{
	int N = conf.svmBudgetSize > 0 ? conf.svmBudgetSize+2 : kMaxSVs;   //特征向量个数 如果没设置就用2000作上限  N=100+2，特征向量的个数不能超过这个阈值  
	m_K = MatrixXd::Zero(N, N);                                       //核矩阵
	m_debugImage = Mat(800, 600, CV_8UC3);                            // debug图像矩阵
	DisGuss.e = {};
	DisGuss.var = {};
}

void LaRank::computeDisGuss(){

	//for (int i = 0; i < m_svs.size(); i++){
	//	for (int j = 0; j < m_svs[i]->x->refCount;j++)
	//		DisGuss.e[j] = DisGuss.e[j] + m_svs[i]->x->x[i][j];
	//}
	//DisGuss.e = DisGuss.e / m_svs.size();
	//for (int i = 0; i < m_svs.size(); i++){
	//	DisGuss.var = DisGuss.var + (m_svs[i]->x->x[i] - DisGuss.e).cross(m_svs[i]->x->x[i] - DisGuss.e);
	//}
	//DisGuss.var = DisGuss.var / m_svs.size();
...
double LaRank::Evaluate(const Eigen::VectorXd& x, const FloatRect& y) const  //论文中公式10 的 F 用来评价占用的
{
	double f = 0.0;
...
void LaRank::Eval(const MultiSample& sample, std::vector<double>& results, vector<double>& p)
{
	...
	for (int i = 0; i < (int)fvs.size(); ++i)
	{
		...
		y.Translate(-centre.XMin(), -centre.YMin());// M_xMin+= x;  m_yMin += y;
		results[i] = Evaluate(fvs[i], y);
	}
}

void LaRank::Update(const MultiSample& sample, int y)   //更新函数
{
	...
	for (int i = 0; i < (int)rects.size(); ++i)
	{
		...
		FloatRect r = rects[i];
		r.Translate(-centre.XMin(), -centre.YMin());//变换 记录到Y中
		sp->yv.push_back(r);
		if (!m_config.quietMode && m_config.debugMode)
		{
			...
			resize(sample.GetImage().GetImage(0)(roi), im, im.size());
			sp->images.push_back(im);
		}
	}
	// evaluate features for each sample 为每个样本评估特征
	sp->x.resize(rects.size());
	const_cast<Features&>(m_features).Eval(sample, sp->x);
	sp->y = y;
	sp->refCount = 0;
	m_sps.push_back(sp); //push_back 在末尾加上sp

	ProcessNew((int)m_sps.size()-1);
	BudgetMaintenance();// 筛选支持向量
...
double LaRank::ComputeDual() const  //计算对偶形式
{
	double d = 0.0;
	for (int i = 0; i < (int)m_svs.size(); ++i)
	{
...
void LaRank::SMOStep(int ipos, int ineg)   //SMO算法
{
	if (ipos == ineg) return;
...
pair<int, double> LaRank::MinGradient(int ind)  //梯度最小化
{
...
int LaRank::AddSupportVector(SupportPattern* x, int y, double g)    //添加支持向量
{
	...
	sv->g = g;
	//int tset = 0;
	//for (int sdsdf = 0; sdsdf < sv->x->refCount; sdsdf++)
	//{
	//	tset++;
	//	cout << sv->x->x[sdsdf].size()<< endl;
	//	//cout << sv->x->x[sdsdf][1] << "  " << sv->x->x[sdsdf][2] << endl;
	//	waitKey(0);
	//}
	//cout << tset << endl;
...
void LaRank::SwapSupportVectors(int ind1, int ind2)  //交换支持向量
{
...
void LaRank::RemoveSupportVector(int ind)   //移除支持向量
{
#if VERBOSE
...
{
	// find negative sv with smallest effect on discriminant function if removed发现负样本中除去后影响最小的支持向量
...
void LaRank::UpdateDebugImage()    //更新debug图像
{
