...
void Tracker::Track(const cv::Mat& frame, FloatRect m_aa)
{
	...
	vector<FloatRect> rects = Sampler::PixelSamples(m_aa, m_config.searchRadius);
	...
	vector<double> gailv;
	m_pLearner->Eval(sample, scores, gailv);      
	...
	//Bestgailv = gailv[bestInd];
	//cout << Bestgailv << endl;
	//waitKey(0);
	UpdateDebugImage(keptRects, m_aa, scores);
	//printf("bestscore:%f \n",bestScore);
	if (bestInd != -1)
	{
